...
func init() {
	android.RegisterModuleType("cyber_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cyberExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cyberExpandVariables(ctx, dep_file)
	...
	cmd := cyberExpandVariables(ctx, String(g.properties.Cmd))
