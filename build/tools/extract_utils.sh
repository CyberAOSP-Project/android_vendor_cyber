...
function setup_vendor() {
    ...
    export CYBER_ROOT="$3"
    if [ ! -d "$CYBER_ROOT" ]; then
        echo "\$CYBER_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$CYBER_ROOT/$OUTDIR" ]; then
        mkdir -p "$CYBER_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$CYBER_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$CYBER_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$CYBER_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The CyberAOSP Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The CyberAOSP Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The CyberAOSP Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The CyberAOSP Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The CyberAOSP Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CYBER_ROOT"/vendor/cyber/build/tools/smali/baksmali.jar
        export SMALIJAR="$CYBER_ROOT"/vendor/cyber/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CYBER_ROOT"/vendor/cyber/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CYBER_ROOT"/vendor/cyber/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$CYBER_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CYBER_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$CYBER_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$CYBER_ROOT"/vendor/cyber/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$CYBER_ROOT"/"$OUTDIR"/radio
