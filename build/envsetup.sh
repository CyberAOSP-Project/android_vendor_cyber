function __print_cyber_functions_help() {
cat <<EOF
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/cyber/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/CyberAOSP-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.cyberaosp.device | grep -q "$CYBER_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $CYBER_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.cyberaosp.device | grep -q "$CYBER_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.cyberaosp.device | grep -q "$CYBER_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cyber/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $CYBER_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/cyber/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
